...
class MailBot(object):
    def __init__(self):
        ...
        self.username = 'bot2mail@163.com'
        self.username = '1763283228@qq.com'
        self.password = 'bot666'
        self.password = 'tiercxfoarxveaad'
    ...
    def login(self, trytimes = 4):
        if not self.downloader_login(trytimes):
            print("mail downloader login fialed")
            return False
        if not self.sender_login(trytimes):
            print("mail sender login failed")
            return False
    ...
    def downloader_login(self, trytimes = 3):
        for i in range(0, trytimes):
            try:
                ...
                print('downloader login {0} success'.format(self.imap_host))
                self.downloader = downloader
                return True
            except Exception as e:
                print('downloader login {0} failed'.format(self.imap_host), e)
    ...
    def sender_login(self, trytimes = 3):
        for i in range(0, trytimes):
            try:
                ...
                self.sender = yag
                print("sender login success to {0}".format(self.smtp_host))
                return True
            except Exception as e:
                print("sender login failed", e)
    ...
    def send_mail(self, email_adr, subject, contents):
        ...
        '''
        if not self.sender:
            print("mail_sender not logining")
            return False
        try:
            self.sender.send(email_adr, subject, contents)
            print("send email to {0} success".format(email_adr))
            return True
        except Exception as e:
            print("send email to {0} failed".format(email_adr))
            return False
    
    def recv_mail(self):
        if not self.downloader:
            print("downloader not login")
            return False
    
        self.downloader.select() # Select inbox or default namespace
        (retcode, messages) = self.downloader.search(None, '(UNSEEN)')
        if retcode != 'OK':
            print("read mail from server failed")
            return False
        ...
        for num in messages[0].split():
            ...
            print("get mail: subject[{0}] body.size[{1}]".format(subject, len(body)))
    ...
    def recv_mail_weibo(self, subject_pattern, from_email_pattern):
        ...
        for num in messages[0].split():
            ...
            # find target email
            body = email_data.get('body')
            print("read email: subject:{0} body size:{1}".format(subject, len(body)))
            vcode = body.split('\n')[0]
            return vcode
    ...
    def handle_mail(self, email_data):
        ...
        print("recv email from {0} subject is :{1}".format(from_email[1], subject))
...
def main():
    mbot = MailBot()
    if not mbot.login(10):
        print("login mail failed")
        return

    vcode = mbot.get_weibo_vcode("weibo_vcode", "nikinumber4@163.com")
    print("get weibo vcode:{0}".format(vcode))
